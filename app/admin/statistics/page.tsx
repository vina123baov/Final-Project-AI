  ...
  if (error) return (
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="text-center max-w-md">
        ...
        <button onClick={fetchData} className="px-6 py-3 bg-primary text-primary-foreground font-bold rounded-lg flex items-center gap-2 mx-auto">
      ...
      <div className="container mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-card border border-border rounded-xl p-6">
            <p className="text-muted-foreground text-sm mb-2">Tổng Yêu Cầu</p>
            <p className="text-3xl font-bold text-foreground">{stats?.total_requests ?? 0}</p>
            <p className="text-sm text-muted-foreground">Thành công: {stats?.total_success ?? 0} | Thất bại: {stats?.total_failed ?? 0}</p>
          </div>
          <div className="bg-card border border-border rounded-xl p-6">
            <p className="text-muted-foreground text-sm mb-2">Thời Gian Xử Lý TB</p>
            <p className="text-3xl font-bold text-foreground">{((stats?.avg_processing_time_ms ?? 0) / 1000).toFixed(1)}s</p>
            <p className="text-sm text-muted-foreground">{(stats?.avg_processing_time_ms ?? 0).toFixed(0)}ms</p>
          </div>
          <div className="bg-card border border-border rounded-xl p-6">
            <p className="text-muted-foreground text-sm mb-2">Confidence TB</p>
            <p className="text-3xl font-bold text-foreground">{((stats?.avg_confidence ?? 0) * 100).toFixed(1)}%</p>
            <p className="text-sm text-muted-foreground">Blur TB: {(stats?.avg_blur_score ?? 0).toFixed(0)}</p>
          </div>
          <div className="bg-card border border-border rounded-xl p-6">
            <p className="text-muted-foreground text-sm mb-2">Người Dùng</p>
            <p className="text-3xl font-bold text-foreground">{stats?.total_active_users ?? 0}</p>
            <p className="text-sm text-muted-foreground">Admin: {stats?.total_admins ?? 0}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          <div className="bg-card border border-border rounded-xl p-6">
            ...
            <div className="space-y-4">
              <div>
                <div className="flex justify-between mb-2"><span className="text-sm">Thành công</span><span className="text-sm font-semibold">{stats?.total_success ?? 0} ({(stats?.success_rate_percent ?? 0).toFixed(1)}%)</span></div>
                <div className="w-full bg-secondary rounded-full h-3"><div className="bg-success h-3 rounded-full" style={{ width: `${stats?.success_rate_percent ?? 0}%` }} /></div>
              </div>
              <div>
                <div className="flex justify-between mb-2"><span className="text-sm">Thất bại</span><span className="text-sm font-semibold">{stats?.total_failed ?? 0}</span></div>
                <div className="w-full bg-secondary rounded-full h-3"><div className="bg-destructive h-3 rounded-full" style={{ width: `${100 - (stats?.success_rate_percent ?? 0)}%` }} /></div>
              </div>
              <div>
                <div className="flex justify-between mb-2"><span className="text-sm">Chờ duyệt</span><span className="text-sm font-semibold">{stats?.total_pending ?? 0}</span></div>
                <div className="w-full bg-secondary rounded-full h-3"><div className="bg-warning h-3 rounded-full" style={{ width: `${(stats?.total_requests ?? 0) > 0 ? ((stats?.total_pending ?? 0) / (stats?.total_requests ?? 1)) * 100 : 0}%` }} /></div>
              </div>
            </div>
          </div>
            ...
            {errors.length > 0 ? (
              <div className="space-y-4">
                {errors.map((err, idx) => {
                  ...
                  return (
                    <div key={idx}>
                      <div className="flex justify-between mb-1"><span className="text-sm">{labels[err.result_type] || err.result_type}</span><span className="text-sm font-semibold">{err.count} ({pct.toFixed(1)}%)</span></div>
                      <div className="w-full bg-secondary rounded-full h-3"><div className="bg-gradient-to-r from-primary to-accent h-3 rounded-full" style={{ width: `${pct}%` }} /></div>
                    </div>
                  )
                })}
              </div>
