    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-sm text-muted-foreground mb-6">Đảm bảo backend Django đang chạy tại http://localhost:8000</p>
          <button onClick={fetchData} className="px-6 py-3 bg-primary text-primary-foreground font-bold rounded-lg hover:opacity-90 transition flex items-center gap-2 mx-auto">
    ...
    <div className="min-h-screen bg-background py-12 px-4">
      <div className="container mx-auto">
        <div className="mb-8 flex items-center justify-between">
          ...
          <button onClick={fetchData} className="p-2 hover:bg-secondary rounded-lg transition text-muted-foreground hover:text-foreground" title="Tải lại">
            <RefreshCw size={20} />
          </button>
        </div>
        ...
        {familiesWithLocations.length > 0 && (
          <div className="bg-card border border-border rounded-xl p-6 mb-8">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-lg font-bold text-foreground">Bản Đồ Phân Bố</h2>
            </div>
            ...
            <div className="space-y-4">
              <div>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm text-foreground">Confidence TB</span>
                  <span className="text-sm font-semibold">{((stats?.avg_confidence ?? 0) * 100).toFixed(1)}%</span>
                </div>
                <div className="w-full bg-secondary rounded-full h-2">
                  <div className="bg-primary h-2 rounded-full" style={{ width: `${(stats?.avg_confidence ?? 0) * 100}%` }} />
                </div>
              </div>
              <div>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm text-foreground">Blur Score TB</span>
                  <span className="text-sm font-semibold">{(stats?.avg_blur_score ?? 0).toFixed(0)}</span>
                </div>
                <div className="w-full bg-secondary rounded-full h-2">
                  <div className="bg-success h-2 rounded-full" style={{ width: `${Math.min(((stats?.avg_blur_score ?? 0) / 500) * 100, 100)}%` }} />
                </div>
              </div>
              <div>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm text-foreground">Tỷ lệ thành công</span>
                  <span className="text-sm font-semibold">{(stats?.success_rate_percent ?? 0).toFixed(1)}%</span>
                </div>
                <div className="w-full bg-secondary rounded-full h-2">
                  <div className="bg-success h-2 rounded-full" style={{ width: `${stats?.success_rate_percent ?? 0}%` }} />
                </div>
              </div>
            </div>
          </div>
                  ...
                  return (
                    <div key={idx}>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-sm">{labels[err.result_type] || err.result_type}</span>
                        <span className="text-sm font-semibold">{err.count} ({pct.toFixed(1)}%)</span>
                      </div>
                      <div className="w-full bg-secondary rounded-full h-2">
