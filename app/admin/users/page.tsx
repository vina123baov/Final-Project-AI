...
export default function AdminUsersPage() {
  ...
  if (loading) return (<><Header /><main className="min-h-screen bg-background flex items-center justify-center"><div className="text-center"><Loader size={48} className="animate-spin text-primary mx-auto mb-4" /><p className="text-muted-foreground">Đang tải danh sách người dùng...</p></div></main><Footer /></>)
  if (error) return (<><Header /><main className="min-h-screen bg-background flex items-center justify-center px-4"><div className="text-center max-w-md"><AlertCircle size={48} className="text-destructive mx-auto mb-4" /><h2 className="text-xl font-bold text-foreground mb-2">Lỗi Kết Nối</h2><p className="text-muted-foreground mb-6">{error}</p><button onClick={fetchUsers} className="px-6 py-3 bg-primary text-primary-foreground font-bold rounded-lg flex items-center gap-2 mx-auto"><RefreshCw size={18} />Thử lại</button></div></main><Footer /></>)
        ...
        <div className="container mx-auto">
          <div className="mb-8 flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground">Dữ liệu thực từ GET /api/admin/users/</p>
            </div>
